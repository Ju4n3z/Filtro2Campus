...
const getUserAll = (async()=>{ //Función para obtener todos los clientes
    ...
    if(res.length !=0){
        ...
        res.forEach(element => {
            ...
            `
            numero = `Cantidad de rutas = ${i+1}`
            cantidad.innerHTML = numero;
            tbody.insertAdjacentHTML("beforeend",tabla);
            i++;
        });
    } else {
        numero = `Cantidad de rutas = ${i}`
        cantidad.innerHTML = numero;
    }
})()

const postUser = async(data)=>{ //Función para registrar una nueva ruta
    config.method = "POST";
...
const deleteUser = async(id)=>{ //Función para eliminar una ruta
    config.method = "DELETE";
    let res = await ( await fetch(`http://localhost:${port}/rutas/${id}`,config)).json();
    console.log(res);
}
